...
public:
    explicit CLIConfiguration(const TFilesystem& filesystem)
        ...
        _options.getOptions().add_options()("help,h", "display help message")(
                ...
                "exclude verification of genuine pairs")(
                "exclude-impostors",
                po::value<bool>()->default_value(false),
                "exclude verification of impostors pairs");
    };
    ...
    void parse(int argc, const char **argv) {
        ...
        try {
            ...
            _options.getDataVariant("config-file") =
                    pathParamValidator("config-file", kConfigFileDefault);
            _options.getDataVariant("verifications") = boolParamValidator("verifications");
            _options.getDataVariant("help") = boolParamValidator("help");
            _options.getDataVariant("output-directory") =
                    pathParamValidator("output-directory", kOutDirectoryDefault);
            _options.getDataVariant("impostors") = pathParamValidator("impostors");
            _options.getDataVariant("genuines") = pathParamValidator("genuines");
            _options.getDataVariant("max-rotation") =
                    intParamValidator("max-rotation", 0, 180);
            _options.getDataVariant("dpi") = intParamValidator("max-rotation", 50, 2000);
            _options.getDataVariant("template-format") =
                    listParamValidator("template-format", {"ansi", "iso"});
            _options.getDataVariant("exclude-genuines") = vm["exclude-genuines"].as<bool>();
            _options.getDataVariant("exclude-impostors") = vm["exclude-impostors"].as<bool>();
    ...
    TResult getData(const std::string& option) { return _options.template getData<TResult>(option); }
    ...
    std::string getXmlPath() const {
        return _options.template getData<std::string>("config-file");
    };
